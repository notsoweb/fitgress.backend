...
| `app/Http/Controllers/Resources/MachineResource.php` | Catálogo de máquinas (`machine:all`) |
...
| `resources.get` / `catalogs.get` | `api.resource({ 'exercise:all' })` en planes y `api.resource({ 'machine:all' })` en el filtro de ejercicios |
